...
(cd "$REPO" && stow --restow aerospace skhd sketchybar borders bin zsh autoraise ghostty)  # .stowrc sets --dir/--target
...
brew services start autoraise  >/dev/null 2>&1 || brew services restart autoraise  >/dev/null 2>&1 || true
...
Grant these by hand (macOS will not let a script do it):
  System Settings > Privacy & Security > Accessibility   -> AeroSpace, borders, AutoRaise
